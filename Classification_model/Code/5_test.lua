...
print('Testing')
print('Testing')
testLogger1 = optim.Logger(paths.concat('../Results/PredictionsNew/Output_Valid.log'))
...
function test()
	...
	for j=1, valid_in:size()[1]-opt.batchSize do
		...
		local pred_test = model:forward(valid_in:sub(j, j+opt.batchSize-1):resize(128))
