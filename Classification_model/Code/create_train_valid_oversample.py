...
data_groups = valid_data.groupby(by='zfret1')
minority = data_groups.get_group(1)

import random
extra_data = random.sample(np.array(minority),689)
oversampled_data = pd.DataFrame(extra_data, columns = data.columns)
valid_data1 = pd.concat([valid_data,oversampled_data])
...
train_data[cols].to_csv('../Data/Train_'+name_tr+'_NS.csv', index = False)
valid_data1[cols].to_csv('../Data/Valid_'+name_vd+'_NS.csv', index = False)
test_data[cols].to_csv('../Data/Test_'+name_ts+'_NS.csv', index = False)
