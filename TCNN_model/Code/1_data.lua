...
if not opt then
   ...
   cmd:text()
   cmd:text('Options:')
   cmd:option('-trainD', '../Data/Train_1992_1993_.csv', 'Train dataset directory+file')
   cmd:option('-validD', '../Data/Valid_1994_1995_.csv', 'Valid dataset directory+file')
   cmd:option('-testD', '../Data/Test_1996_.csv', 'Train dataset directory+file')
   cmd:text()
...
org_train_in = train_tensor:narrow(2,1,131):clone()
org_valid_in = valid_tensor:narrow(2,1,131):clone()
org_test_in = test_tensor:narrow(2,1,2):clone()


train_out = train_tensor:narrow(2,132,1):clone()
valid_out = valid_tensor:narrow(2,132,1):clone()
